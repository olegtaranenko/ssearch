...
/* ---------------------------------------------------------------------- */
	...
	/* ---------------------------------------------------------------------- */
	// ChromeConsoleAppender
	// @author olegtaranenko@gmail.com

	function ChromeConsoleAppender() {}

	ChromeConsoleAppender.prototype = new log4javascript.Appender();
	ChromeConsoleAppender.prototype.layout = new NullLayout();
	ChromeConsoleAppender.prototype.threshold = Level.DEBUG;

	ChromeConsoleAppender.prototype.append = function(loggingEvent) {
		var appender = this;
		...
		if (console && console.log) {
			...
			if (console.debug && Level.DEBUG.isGreaterOrEqual(loggingEvent.level)) {
				console.debug.apply(console, formattedMessage);
			} else if (console.info && Level.INFO.equals(loggingEvent.level)) {
				console.info.apply(console, formattedMessage);
			} else if (console.warn && Level.WARN.equals(loggingEvent.level)) {
				console.warn.apply(console, formattedMessage);
			} else if (console.error && loggingEvent.level.isGreaterOrEqual(Level.ERROR)) {
				console.error.apply(console, formattedMessage);
			} else {
				console.log.apply(console, formattedMessage);
			}
		}
	};

	ChromeConsoleAppender.prototype.group = function(name) {
		if (console && console.group) {
			console.group(name);
		}
	};

	ChromeConsoleAppender.prototype.groupEnd = function() {
		if (console && console.groupEnd) {
			console.groupEnd();
		}
	};

	ChromeConsoleAppender.prototype.toString = function() {
		return "ChromeConsoleAppender";
	};

	log4javascript.ChromeConsoleAppender = ChromeConsoleAppender;
